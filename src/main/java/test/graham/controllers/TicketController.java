        ...
        System.out.println(Driver.currentLoggedEmployee);
            if(Driver.currentLoggedEmployee != null) {
                try {
                    ...
                    ctx.result("you did not enter a description or amount");
                }
            }
    ...
    public Handler updateTicket = (ctx) -> {
        try {
            if (Driver.currentLoggedEmployee.getIsAdmin() == 1) {
                ...
                Gson gson = new Gson();
                Ticket ticketId = gson.fromJson(ticketJSON, Ticket.class);
                Ticket ticket = Driver.ticketService.updatePendingTicket(ticketId);
